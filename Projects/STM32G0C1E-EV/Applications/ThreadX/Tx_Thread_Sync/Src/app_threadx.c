...
static VOID Led_Toggle(GPIO_TypeDef *GPIOx, uint16_t GPIO_PIN_x, UINT iter);
...
void ThreadOne_Entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    if (APP_SYNC_GET(&SyncObject, TX_NO_WAIT) == TX_SUCCESS)
    {
      ...
      Led_Toggle(GPIOD, GPIO_PIN_5, 10);
...
void ThreadTwo_Entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    if (APP_SYNC_GET(&SyncObject, TX_NO_WAIT) == TX_SUCCESS)
    {
      ...
      /*Sync object acquired toggle the LED_ORANGE each 500ms for 5s*/
      Led_Toggle(GPIOD, GPIO_PIN_6, 10);
